...
public static class ErrorCodes
{
    ...
    /// <summary>
    /// Caller tried to retry an import that isn't in the <c>Failed</c>
    /// terminal state. HTTP 409. The retry endpoint is the only legal
    /// way to push a Failed import back to Queued; calling it against a
    /// Pending / Running / Done row is a no-op-or-worse, so we reject
    /// with this distinct code instead of a generic 400 so the frontend
