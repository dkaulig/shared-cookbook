...
/// </summary>
public class ImportEndpointsTests : IClassFixture<SharedCookbookWebApplicationFactory>, IAsyncLifetime
{
    private readonly SharedCookbookWebApplicationFactory _factory;
    private HttpClient _client = null!;

    public ImportEndpointsTests(SharedCookbookWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    /// <see cref="SharedCookbook.Api.Services.ImageSigningService"/>).
    /// </summary>
    private string SignPhotoUrl(string path)
    {
        using var scope = _factory.Services.CreateScope();
        var signer = scope.ServiceProvider
            .GetRequiredService<SharedCookbook.Api.Services.ImageSigningService>();
