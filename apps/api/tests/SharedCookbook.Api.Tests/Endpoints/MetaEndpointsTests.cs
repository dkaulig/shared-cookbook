...
/// its own <see cref="SharedCookbookWebApplicationFactory"/> so the
/// <c>WithAiConfig</c> override doesn't bleed across tests via a shared
/// host instance.
/// </summary>
public class MetaEndpointsTests
{
    private static async Task<(SharedCookbookWebApplicationFactory factory, HttpClient client)> BuildAsync(
        bool enabled, string provider)
    {
        var factory = new SharedCookbookWebApplicationFactory()
