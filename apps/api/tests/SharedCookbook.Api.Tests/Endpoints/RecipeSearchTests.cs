...
/// </summary>
public class RecipeSearchTests : IClassFixture<SharedCookbookWebApplicationFactory>, IAsyncLifetime
{
    private readonly SharedCookbookWebApplicationFactory _factory;
    private HttpClient _client = null!;

    public RecipeSearchTests(SharedCookbookWebApplicationFactory factory)
    {
        _factory = factory;
    }
        ...
        DateTimeOffset? updatedAt = null)
    {
        ...
        var recipe = new SharedCookbook.Domain.Entities.Recipe(
            groupId, createdByUserId, title, description, 4, null, 1, null,
            SharedCookbook.Domain.Enums.RecipeSourceType.Manual, null, now);
        db.Recipes.Add(recipe);
        if (tagIds is not null)
        {
            foreach (var tagId in tagIds)
            {
                db.RecipeTags.Add(new SharedCookbook.Domain.Entities.RecipeTag(recipe.Id, tagId));
            }
        }
        await db.SaveChangesAsync();
        return recipe.Id;
    ...
    private async Task<Guid> SeedGroupTagAsync(Guid groupId, Guid createdBy, string name)
    {
        using var scope = _factory.Services.CreateScope();
        var db = scope.ServiceProvider.GetRequiredService<AppDbContext>();
        var tag = SharedCookbook.Domain.Entities.Tag.CreateGroupScoped(createdBy, groupId, name);
        db.Tags.Add(tag);
        await db.SaveChangesAsync();
        return tag.Id;
    }
    ...
    public async Task Search_Authz_DoesNotLeak_Recipes_From_Groups_Caller_Is_Not_In()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            db.GroupMemberships.Add(new SharedCookbook.Domain.Entities.GroupMembership(
                callerId, gA.GroupId, SharedCookbook.Domain.Enums.GroupRole.Member, now));
            db.GroupMemberships.Add(new SharedCookbook.Domain.Entities.GroupMembership(
                callerId, gB.GroupId, SharedCookbook.Domain.Enums.GroupRole.Member, now));
            await db.SaveChangesAsync();
        }
