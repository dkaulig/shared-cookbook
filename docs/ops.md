...
(CPX41, 16 GB RAM, 8 vCPU). Host: `kochbuch.kaulig.dev`, deploy user:
...
umask 077
scp deploy@kochbuch.kaulig.dev:/srv/shared-cookbook/.env \
...
orchestrator bot (`orchestrator@kochbuch.kaulig.dev`, role `User`)
...
8. **Apply legacy-DB SQL migrations.** Two `UPDATE`s for previously
   ...
   ```bash
   scp migrate.sql deploy@kochbuch.kaulig.dev:/tmp/migrate.sql
   ssh deploy@kochbuch.kaulig.dev \
